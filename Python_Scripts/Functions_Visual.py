...
def Draw_Figure(SatisfiedRegions,UnsatisfiedRegions, Height, Width, name = 'Satisfaction_Assessment'):

    data=[[[255 for i in range(3)] for j in range(Width)] for l in range(Height)]
    for region in SatisfiedRegions:
        for section in region.sections:
            for s in range(section.s_size):
                data[section.s_line][section.s_beg + s] = [0, 128, 0]

    for region in UnsatisfiedRegions:
        for section in region.sections:
            for s in range(section.s_size):
                data[section.s_line][section.s_beg + s] = [255, 0, 0]

    data2=np.array(data, dtype=np.uint8)

    img = Image.fromarray(data2, 'RGB')
    img.save(Results_Directory() + '/{}.png'.format(name))
    img.show()
